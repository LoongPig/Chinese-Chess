...
namespace N_Board{
    ...
    private:
        Chess board[15][15];
        int StepCnt;
        coord<int> redMaster,blackMaster;// Place of them
    public:
        ...
        wchar_t gChess(coord<int> _){ return gChess(_.x,_.y); }
        Color gColor(int x,int y){ return board[x][y].gColor(); }
        Color gColor(coord<int> _){ return gColor(_.x,_.y); }
        coord<int> gMaster(Color col){// get the master place
            if(col==red) return redMaster;
            return blackMaster; 
        }
        void changeMaster(Color col,int x,int y){// change the place
            if(col==red) redMaster=coord<int>(x,y);
            else blackMaster=coord<int>(x,y);
        }
        void Move(int x,int y,int a,int b){ board[a][b].change(board[x][y]);board[x][y].change(Chess()); }//  x,y => a,b
        void Move(coord<int> s,coord<int> e){ Move(s.x,s.y,e.x,e.y); }
        ...
        Board(){
            StepCnt=0,redMaster=coord<int>(1,5),blackMaster=coord<int>(10,5);
            Change(
        ...
        coord<int> selectChess(){// select a chess
            static coord<int> pla={1,1};
        ...
        bool checkWay(coord<int> s,coord<int> e){// check the chess way is it ok.
            ...
            for(int i=1;i<=10;i++)
                for(int j=1;j<=9;j++)
                    if(gColor(i,j)==gOppoColor(col))
                        if(checkWay(coord<int>(i,j),gMaster(col))) return true;
            return false;
        }
